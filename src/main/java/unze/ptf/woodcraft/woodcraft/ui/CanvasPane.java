...
public class CanvasPane extends Pane {
    ...
    public void setMode(Mode mode) {
        Mode resolved = resolveMode(mode);
        this.mode = resolved;
        if (resolved == Mode.PEN) {
            setCursor(Cursor.CROSSHAIR);
        } else if (resolved == Mode.SELECT) {
            setCursor(Cursor.HAND);
        } else if (resolved == Mode.ERASE) {
    ...
    private Mode resolveMode(Mode mode) {
        if (mode == Mode.ADD_NODE || mode == Mode.CONNECT_NODES) {
            return Mode.PEN;
        }
        if (mode == Mode.NONE) {
            return Mode.SELECT;
